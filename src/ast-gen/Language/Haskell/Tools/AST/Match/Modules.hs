...
pattern ModuleHead :: Ann ModuleName dom stage -> AnnMaybe ExportSpecList dom stage 
...
pattern ModuleExport :: Ann ModuleName dom stage -> Ann ExportSpec dom stage
...
pattern IESpec :: Ann Name dom stage -> AnnMaybe SubSpec dom stage -> Ann IESpec dom stage
pattern IESpec name ss <- Ann _ (UIESpec name ss)

pattern SubList :: AnnList Name dom stage -> Ann SubSpec dom stage
...
pattern ImportDecl :: AnnMaybe ImportSource dom stage -> AnnMaybe ImportQualified dom stage 
                        -> AnnMaybe ImportSafe dom stage -> AnnMaybe StringNode dom stage
                        -> Ann ModuleName dom stage -> AnnMaybe ImportRenaming dom stage
...
pattern ImportRenaming :: Ann ModuleName dom stage -> Ann ImportRenaming dom stage
