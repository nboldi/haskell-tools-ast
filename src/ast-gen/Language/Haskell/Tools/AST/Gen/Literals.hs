...
mkCharLit :: Char -> Ann ULiteral dom SrcTemplateStage
mkCharLit c = mkAnn (fromString $ show c) $ UCharLit c

mkStringLit :: String -> Ann ULiteral dom SrcTemplateStage
mkStringLit s = mkAnn (fromString $ show s) $ UStringLit s

mkIntLit :: Integer -> Ann ULiteral dom SrcTemplateStage
mkIntLit i = mkAnn (fromString $ show i) $ UIntLit i

mkFracLit :: Rational -> Ann ULiteral dom SrcTemplateStage
mkFracLit f = mkAnn (fromString $ show f) $ UFracLit f

mkPrimIntLit :: Integer -> Ann ULiteral dom SrcTemplateStage
mkPrimIntLit i = mkAnn (fromString $ show i ++ "#") $ UPrimIntLit i

mkPrimWordLit :: Integer -> Ann ULiteral dom SrcTemplateStage
mkPrimWordLit i = mkAnn (fromString $ show i ++ "##") $ UPrimWordLit i

mkPrimFloatLit :: Rational -> Ann ULiteral dom SrcTemplateStage
mkPrimFloatLit f = mkAnn (fromString $ show f ++ "#") $ UPrimFloatLit f

mkPrimDoubleLit :: Rational -> Ann ULiteral dom SrcTemplateStage
mkPrimDoubleLit f = mkAnn (fromString $ show f ++ "##") $ UPrimDoubleLit f

mkPrimCharLit :: Char -> Ann ULiteral dom SrcTemplateStage
mkPrimCharLit c = mkAnn (fromString $ show c ++ "#") $ UPrimCharLit c

mkPrimStringLit :: String -> Ann ULiteral dom SrcTemplateStage
